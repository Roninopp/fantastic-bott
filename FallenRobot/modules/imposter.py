...
# Function to send message to all enabled groups
async def send_imposter_message(msg):
    for group_id in enabled_groups:
        await kirimPesan(group_id, msg)
...
async def cek_mataa(_, m):
    ...
    if not await cek_userdata(m.from_user.id):
        ...
        msg = ""
        old_user = await app.get_chat_member(group_id, m.from_user.id)
        ...
        if last_name != m.from_user.last_name:
            ...
            await add_userdata(m.from_user.id, m.from_user.username, m.from_user.first_name, m.from_user.last_name)
        if msg != "":
            await send_imposter_message(msg)
...
async def set_mataa(_, m):
    ...
    if m.command[1] == "on":
        ...
        if cekset:
            ...
            enabled_groups.append(m.chat.id)  # Add the group to the enabled group list
            await kirimPesan(m, "Imposter Detection enabled in your group. I will track name and username changes in this chat. If user changes their name and username, I will send a message showing any related changes")
        ...
        if not cekset:
            ...
            enabled_groups.remove(m.chat.id)  # Remove the group from the enabled group list
...
__help__ = """
*• /detectimposter:* Use this command to track name and username changes in group. If a user changes their name and username, I will send a message showing any related changes.
"""
