...
async def _(event):
    ...
    is_admin = False
    try:
        GabiBraunRobot = await telethn(GetParticipantRequest(event.chat_id, event.sender_id))
    except UserNotParticipantError:
        is_admin = False
    else:
        if isinstance(
            GabiBraunRobot.participant,
            (
                ChannelParticipantAdmin,
                ChannelParticipantCreator,
            ),
...
__mod_name__ = "ᴀᴅᴠᴀɴᴄᴇᴅ"
__help__ = """

➩ /unbanall : Unban All Members From Group  **!note Only Group Admin Or Owner can do**

➩ /unmuteall : Unmute All Members From Group  **Only Group Owner Or Admin Can do**

➩ /users : Get Group Users List

➩ /figlet : Make A Figlet On The Given Text 

𝐄𝐱𝐚𝐦𝐩𝐥𝐞 `/figlet Ronin`
                
➩ /hastag : You Can Use This HasTag Generator Which Will Give You The Top 30 And More HasTags Based Off One Keyboard Selection
